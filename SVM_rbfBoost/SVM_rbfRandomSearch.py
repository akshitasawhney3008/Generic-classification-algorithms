...
for key, value in dict_of_common_genes_with_k.items():
    if float(key) > 0.41 and float(key) <0.58:
        ...
        for i in range(iter):
            ...
            r = Parallel(n_jobs=20, verbose=5)(delayed(Stratified_kfold)(X_train,Y_train,combination) for combination in randomCombinations)
...
with open('new_dict_of_randomsearch_bestparams_rbf(0.41-0.57).pkl', 'wb') as f:
